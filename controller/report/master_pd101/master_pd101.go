...
func JsonRH101(c *fiber.Ctx) error {
    ...
    filename := fmt.Sprintf("%s_%s_%s_RH101.json", facilityCode, ds1, ds2)
...
// @Security BearerAuth
// @Success 200
// @Router /api/master-pd101/export/rpt1 [get]
func JsonPD101(c *fiber.Ctx) error {
    ...
    facilityCode := config.Config("facilityCode")
    filename := fmt.Sprintf("%s_%s_PD101.json", ds1, ds2)
...
// @Security BearerAuth
// @Success 200
// @Router /api/master-pd101/export/rpt1/xlsx [get]
...
// @Security BearerAuth
// @Success 200
// @Router /api/master-pd101/rpt1 [get]
...
// @Security BearerAuth
// @Success 200
// @Router /api/master-pd101/rpt1 [post]
func SearchList(c *fiber.Ctx) error {
    ...
    data := dto.ReportQueryDto{}
    if err := c.BodyParser(&data); err != nil {
        return fiber.NewError(fiber.StatusBadRequest, err.Error())
    }

    errs := u.ValidatePayload(data, c)
    if errs != nil {
...
// @Security BearerAuth
// @Success 200
// @Router /api/master-pd101/rpt1/{id} [get]
...
// @Security BearerAuth
// @Success 200
// @Router /api/master-pd101/rpt1/{id} [put]
...
func getDb(cli *mongo.Client, vt string) *mongo.Database {
    ...
    if vt == "0" {
        s := fmt.Sprintf("master_pd101%s", suffix)
        db = cli.Database(s)
    } else {
        s := fmt.Sprintf("master_rh101%s", suffix)
        db = cli.Database(s)
    }
